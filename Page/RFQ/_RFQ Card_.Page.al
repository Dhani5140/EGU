...
//                 ApplicationArea = All;
//                 //SubPageLink = "RFQ No." = field("RFQ No.");
//                 SubPageLink = "Vendor No." = field("Vendor No."), "RFQ No." = field("RFQ No."), "Vendor Name" = field("Vendor Name");
    ...
    actions
    {
        area(Processing)
        {
            ...
            action("Create PO")
            {
                ...
                // begin
                //     CurrPage.UPDATE();
                //     IF NOT (Rec.Status IN [Rec.Status::Released, Rec.Status::"Send To Vendor", Rec.Status::Processed]) THEN ERROR('Status must be released or process to create purchase order');
                //     lCURFQFunction.createPOHeader_RFQ(Rec);
                //     CurrPage.UPDATE();
                // end;
                trigger OnAction()
                var
                    lCURFQFunction: Codeunit "RFQ Function";
                begin
                    IF NOT (Rec.Status IN [Rec.Status::Released, Rec.Status::"Send To Vendor", Rec.Status::Processed]) THEN
                        ERROR('Status must be Released, Send To Vendor, or Processed to create PO.');
